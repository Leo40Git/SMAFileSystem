...
#ifdef _WINDEF_
/// Auto-generates a window_handle() call GML-side.
typedef HWND GAME_HWND;
#endif

/// Auto-generates an asset_get_index("argument_name") call GML-side.
typedef int gml_asset_index_of;
/// Wraps a C++ pointer for GML.
template <typename T> using gml_ptr = T*;
/// Same as gml_ptr, but replaces the GML-side pointer with a nullptr after passing it to C++.
...
/// Same as gml_id, but replaces the GML-side ID with a 0 after passing it to C++.
...
public:
	...
	inline int32_t tell() const { return _tell; }
...
#if ((defined(_MSVC_LANG) && _MSVC_LANG >= 201703L) || __cplusplus >= 201703L)
	template<class T> std::optional<T> read_optional() {
		if (read<bool>()) {
			return read<T>;
		}
		else return {};
...
#if ((defined(_MSVC_LANG) && _MSVC_LANG >= 201703L) || __cplusplus >= 201703L)
	template<class T> void write_optional(std::optional<T>& val) {
